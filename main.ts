...
function createHtmlElementForPrayer(
  ...
  for (let x = 1; x < prayers.length; x++) {
    ...
    if (!prayers[x] || prayers[x] === ' ') continue;//we escape the empty strings if the text is not available in all the button's languages
    if (
      actorClass &&
...
async function showTitlesInRightSideBar(
  titlesCollection: NodeListOf<Element>,
...
function showChildButtonsOrPrayers(
  ...
  if (btn.prayers && btn.prayersArray && btn.languages && btn.showPrayers) showPrayers(btn, true, true, containerDiv);
...
//If no onClick parameter/argument is passed to createBtn(), and the btn has any of the following properties: children/prayers/onClick or inlinBtns, we set the onClick parameter to a function passing the btn to showChildButtonsOrPrayers
  if(!onClick && (btn.children || btn.prayers || btn.onClick || btn.inlineBtns)) onClick = () => showChildButtonsOrPrayers(btn, clear);
      ...
      let inlineBtn: Button = new Button({
        ...
        onClick: () => {
          ...
          let createdElements: NodeListOf<Element> =
            containerDiv.querySelectorAll(getDataRootSelector(inlineBtn.prayers[0])
            );
    ...
    btn = createBtn(
      ...
      'editingMode' + localStorage.editingMode.toString(),
        ...
        undefined,
        {
          event: "click",
          fun: () => {
                let tablesArray:string[][][] = eval(prompt('Provide the Name of the Array you Want to Edit', 'testEditingArray'));
            if (tablesArray) {
...
position: { beforeOrAfter: InsertPosition; el: HTMLElement }
  ...
  let createdElements:HTMLElement[] =[], created:HTMLDivElement
prayers.map((p) => {
p.map((row) => {
